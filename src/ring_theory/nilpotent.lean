...
This is about nilpotent elements in rings, mostly aimed at
...
import algebra.ring algebra.commute algebra.group_power algebra.geom_sum
...
/- This block gives a definitions and results that
 are applicable to not-necessarily-commutative rings.
-/
...
def as_nilpotent_neg {a : A} (ha : as_nilpotent a) : as_nilpotent (-a) :=
⟨ha.1, by { rw [pow_neg, ha.2, mul_zero] }⟩

lemma as_nilpotent_neg_exp {a : A} (ha : as_nilpotent a) :
  (as_nilpotent_neg ha).1 = ha.1 := rfl

lemma is_nilpotent_neg {b : A} : is_nilpotent b → is_nilpotent (-b) :=
λ ⟨hb⟩, ⟨as_nilpotent_neg hb⟩

/-- Sum of commuting nilpotent elements is nilpotent -/
lemma nilpotent_add_aux_comm {a b : A} (hab : commute a b) {n m : ℕ}
(ha : a ^ n = 0) (hb : b ^ m = 0) : (a + b) ^ (exp_mul n m) = 0 :=
begin
  have hz : (1 : A) = 0 → (∀ (x : A), x = 0) :=
    λ h x, by { rw [← mul_one x, h, mul_zero] },
  rcases n with ⟨_|n⟩,
  { rw [pow_zero] at ha, exact hz ha _ },
  rcases m with ⟨_|m⟩,
  { rw [pow_zero] at hb, exact hz hb _ },
  have : exp_mul n.succ m.succ = n + m + 1 := rfl,
  rw [this, add_pow_comm hab],
  rw [← @finset.sum_const_zero ℕ A (finset.range (n + m + 1).succ)],
  congr, ext i,
  by_cases hi : i ≥ n + 1,
  { rw [← nat.add_sub_of_le hi , pow_add , ha],
    repeat {rw [zero_mul]} },
  { replace hi := le_of_not_gt hi,
    ...
    rw [this, pow_add, hb, zero_mul, mul_zero, zero_mul] }
end

def as_nilpotent_add_comm {a b : A}
  (hab : commute a b) (ha : as_nilpotent a) (hb : as_nilpotent b) :
  as_nilpotent (a + b) :=
⟨exp_mul ha.val hb.val, nilpotent_add_aux_comm hab ha.2 hb.2⟩

lemma as_nilpotent_add_comm_exp {a b : A}
(hab : commute a b) (ha : as_nilpotent a) (hb : as_nilpotent b) :
...
def as_nilpotent_smul_comm {a b : A} (hab : commute a b) (hb : as_nilpotent b) :
  as_nilpotent (a * b) :=
⟨hb.1 , by { rw [mul_pow_comm hab hb.1, hb.2, mul_zero] }⟩

lemma as_nilpotent_smul_comm_exp {a b : A} (hab : commute a b) (hb : as_nilpotent b) :
...
def as_nilpotent_sub_comm {a b : A}
  (hab : commute a b) (ha : as_nilpotent a) (hb : as_nilpotent b) :
  as_nilpotent (a - b) :=
as_nilpotent_congr (sub_eq_add_neg a b)
  (as_nilpotent_add_comm hab.neg ha (as_nilpotent_neg hb))

lemma as_nilpotent_sub_comm_exp {a b : A}
  (hab : commute a b) (ha : as_nilpotent a) (hb : as_nilpotent b) :
  (as_nilpotent_sub_comm hab ha hb).1 = exp_mul ha.1 hb.1 := rfl

lemma is_nilpotent_sub_comm {a b : A} :
  commute a b → is_nilpotent a → is_nilpotent b → is_nilpotent (a - b) :=
λ hab ⟨ha⟩ ⟨hb⟩, ⟨as_nilpotent_sub_comm hab ha hb⟩
...
/- This block gives a definitions and results for commutative rings.
-/
...
as_nilpotent_smul_comm (all_commute a b) hb
...
def as_nilpotent_sub {a b : A}
  (ha : as_nilpotent a) (hb : as_nilpotent b) : as_nilpotent (a - b) :=
as_nilpotent_sub_comm (all_commute a b) ha hb

lemma as_nilpotent_sub_exp {a b : A}
  (ha : as_nilpotent a) (hb : as_nilpotent b) :
  (as_nilpotent_sub ha hb).1 = exp_mul ha.1 hb.1 :=
as_nilpotent_sub_comm_exp (all_commute a b) ha hb
